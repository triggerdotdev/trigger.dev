...
- A mandatory not-found sweep IS gather, however many rounds it takes, and doubles as the test round — its result is the finding. It never makes the card unaffordable: right after it, render the card, then the verdict.
...
- Never state a cause, a fix, or a dead end in prose while the card says in_progress or doesn't exist yet.
