...
export const runQuerySchema = tool({
  description:
    "Run a read-only TRQL query against the current environment's analytics data and return the result rows. TRQL is a SQL-style language over ClickHouse: bucket time with toStartOfHour/toStartOfDay on the table's time column for time series, and use countIf/sumIf to produce one numeric column per series. Always call get_query_schema first — column names are snake_case (created_at, completed_at, task_identifier); camelCase columns do not exist. Results are capped, so keep queries aggregated. To chart the result, follow with a render_view chart block.",
  inputSchema: z.object({
    query: z
      .string()
      .describe(
