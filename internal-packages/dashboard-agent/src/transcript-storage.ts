...
export function dashboardAgentTranscriptStorage(
  ...
  return {
    async load(scope, opts) {
      // Scope the chat lookup by tenant, so a mismatched clientData cannot read
      // another org or user's transcript even with a valid chatId. The message
      // reads below run only after this row check passes.
      const chat = await db
        .select({ state: chats.transcriptState, cursors: chats.transcriptCursors })
        .from(chats)
        .where(
          and(
            eq(chats.id, scope.chatId),
            eq(chats.organizationId, scope.clientData.organizationId),
            eq(chats.userId, scope.clientData.userId),
