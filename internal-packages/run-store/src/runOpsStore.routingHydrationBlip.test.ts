...
function makeRouter(prisma14: PrismaClient, newClient: RunOpsPrismaClient, onRetry: () => void) {
...
heteroRunOpsBlipTest(
  ...
  async ({ prisma14, prisma17 }) => {
    ...
    // Inject a one-shot connection loss on the token DB's cross-DB hydration read (findMany on the NEW
    // store) so the retry provably fires and recovers deterministically, without racing a live sever:
    // the leaf edge read on prisma14 succeeds, the hydration's first findMany on prisma17 is severed,
    // and the reissue on the retry returns the token.
    let retries = 0;
    const faultingNew = withOneShotBlip(prisma17, "waitpoint", "findMany");
    const router = makeRouter(prisma14, faultingNew as RunOpsPrismaClient, () => {
      retries++;
    });
    ...
    })) as Array<{ waitpoint?: { id?: string } | null }>;
