...
redisTest("emitter -> consumer round trip maps rows, dedups, and acks", async ({ redisOptions }) => {
  const definition = definitionFor("rt");
  const emitter = new MetricsStreamEmitter({ redis: redisOptions, definition, flag: { enabled: () => true } });
  ...
  expect(rows).toContainEqual(expect.objectContaining({ op: "started", q: "queueB", wait: "42" }));
...
redisTest("reclaims stale pending entries from a dead consumer", async ({ redisOptions }) => {
  ...
  await admin.xreadgroup("GROUP", definition.consumerGroup, "zombie", "COUNT", 10, "STREAMS", key, ">");
...
redisTest("per-stream batches: one insert + distinct dedup token per shard stream", async ({ redisOptions }) => {
...
redisTest("probe reports lag as null (not 0) when Redis cannot compute it", async ({ redisOptions }) => {
...
redisTest("emitGauge XADDs an op=gauge snapshot onto the shared metrics stream", async ({ redisOptions }) => {
...
redisTest("sampledSync gates on both the flag and the sample rate", async ({ redisOptions }) => {
  ...
  const off = new MetricsStreamEmitter({ redis: redisOptions, definition, flag: { enabled: () => true }, gaugeSampleRate: 0 });
  const on = new MetricsStreamEmitter({ redis: redisOptions, definition, flag: { enabled: () => true }, gaugeSampleRate: 1 });
  const disabled = new MetricsStreamEmitter({ redis: redisOptions, definition, flag: { enabled: () => false }, gaugeSampleRate: 1 });
...
redisTest("streamState reports depth, lag, and pending per shard", async ({ redisOptions }) => {
  ...
  await admin.xreadgroup("GROUP", definition.consumerGroup, "reader", "COUNT", 1, "STREAMS", key, ">");
