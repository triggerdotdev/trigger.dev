...
class FaultCompletionStore extends PostgresRunStore {
  ...
  // The completion write goes through markWaitpointCompleted (the completion-specific store method),
  // so the fault is injected there - that is the write the guard arms around and replays.
  override async markWaitpointCompleted(
    waitpointId: Parameters<PostgresRunStore["markWaitpointCompleted"]>[0],
    completion: Parameters<PostgresRunStore["markWaitpointCompleted"]>[1]
  ): ReturnType<PostgresRunStore["markWaitpointCompleted"]> {
    if (this.faultsRemaining > 0 && this.mode !== "healthy") {
      this.faultsRemaining--;
      if (this.mode === "commitThenThrow") {
        await super.markWaitpointCompleted(waitpointId, completion); // real commit, then ack "lost"
      }
      throw Object.assign(
