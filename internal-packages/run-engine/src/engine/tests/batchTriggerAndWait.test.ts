...
describe("RunEngine batchTriggerAndWait", () => {
  ...
  containerTest(
    "batch ID should not carry over to blockRunWithWaitpoint",
    async ({ prisma, redisOptions }) => {
      //create environment
      const authenticatedEnvironment = await setupAuthenticatedEnvironment(prisma, "PRODUCTION");
      ...
      try {
        ...
        // Create a regular child to get its waitpoint
        const regularChild = await engine.trigger(
          {
            number: 2,
            friendlyId: "run_c12345",
            environment: authenticatedEnvironment,
        ...
        // Get the child's waitpoint
        const childRunWithWaitpoint = await prisma.taskRun.findUniqueOrThrow({
          where: { id: regularChild.id },
          include: {
            associatedWaitpoint: true,
          },
        });

        // Block the parent with the child's waitpoint
        ...
        expect(parentExecutionData.batch).toBeUndefined();
      } finally {
        engine.quit();
      }
    }
