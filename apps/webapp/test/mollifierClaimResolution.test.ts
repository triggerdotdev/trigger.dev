...
// Stub `~/db.server` before importing the concern â€” the real module
...
function makeConcern(prisma: { findFirst: () => Promise<unknown> }) {
  return new IdempotencyKeyConcern(
    ...
    {} as never, // engine â€” unused on this path
    {} as never, // traceEventConcern â€” unused on this path
  );
}
...
describe("IdempotencyKeyConcern Â· claim resolution", () => {
  it("resolved-but-unfindable falls through to a fresh trigger (no cached run, no claim held)", async () => {
    ...
    // the resolved-but-unfindable terminal case â€” the concern must fall
    ...
    if (result.isCached === false) {
      // No claim held â€” we resolved someone else's (stale) claim, we did
  ...
  it("non-opted-in org skips claimOrAwait entirely (no buffer round-trip, no claim held)", async () => {
    ...
    // mollifier â€” even when `TRIGGER_MOLLIFIER_ENABLED=1` globally and
    ...
    try {
      ...
      expect(result.isCached).toBe(false);
      if (result.isCached === false) {
        // No claim returned â€” the caller must NOT publish/release.
        expect(result.claim).toBeUndefined();
        expect(result.idempotencyKey).toBe("k-1");
      }
