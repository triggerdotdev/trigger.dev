...
const groups: WorkerGroupRegionRow[] = [
  { masterQueue: "us-east-1", region: "us-east-1", workloadType: "CONTAINER", hidden: false, enableFastPath: false },
  { masterQueue: "us-east-1-next", region: "us-east-1", workloadType: "MICROVM", hidden: false, enableFastPath: true },
  { masterQueue: "eu-central-1", region: "eu-central-1", workloadType: "CONTAINER", hidden: false, enableFastPath: false },
];
...
describe("regionForQueue", () => {
  ...
  it("passes through when a group has no region set", () => {
    expect(regionForQueue("x", [{ masterQueue: "x", region: null, workloadType: "CONTAINER", hidden: false, enableFastPath: false }])).toBe("x");
  });
});

describe("backingForQueue", () => {
  it("finds the MICROVM backing for a region with one", () => {
    expect(backingForQueue("us-east-1", groups)).toEqual({ workerQueue: "us-east-1-next", enableFastPath: true });
  });
  ...
  it("returns undefined when the queue's group has no region", () => {
    expect(backingForQueue("x", [{ masterQueue: "x", region: null, workloadType: "CONTAINER", hidden: false, enableFastPath: false }])).toBeUndefined();
  });
  it("ignores hidden MICROVM groups", () => {
    const g: WorkerGroupRegionRow[] = [
      { masterQueue: "us-east-1", region: "us-east-1", workloadType: "CONTAINER", hidden: false, enableFastPath: false },
      { masterQueue: "us-east-1-next", region: "us-east-1", workloadType: "MICROVM", hidden: true, enableFastPath: true },
    ];
