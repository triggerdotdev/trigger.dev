...
import { anonymousConcurrencyLimitQueueName } from "~/v3/services/concurrencyLimitNames.server";
...
describe("ConcurrencyLimitsSystem", () => {
  ...
  postgresTest(
    "an anonymous task limit with a hashed row name resolves by its documented task/<task-id> name",
    async ({ prisma }) => {
      const { environment, system, authEnv } = await seedEnvAndLimit(prisma, {});
      const rowName = anonymousConcurrencyLimitQueueName("billing.charge");
      expect(rowName).not.toBe("limit/task/billing.charge");
      await prisma.taskQueue.create({
        data: {
          friendlyId: `queue_${environment.id}_anon`,
          name: rowName,
          orderableName: rowName,
          projectId: environment.projectId,
          runtimeEnvironmentId: environment.id,
          role: "LIMIT",
          concurrencyVersion: "V2",
          totalConcurrencyLimit: 3,
        },
      });

      const result = await system.limits.retrieve(authEnv, "task/billing.charge");
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        expect(result.value.total).toMatchObject({ current: 3 });
      }
    }
  );
  ...
  postgresTest(
    ...
    async ({ prisma }) => {
      const { system, authEnv } = await seedEnvAndLimit(prisma, { total: 200 });

      const result = await system.limits.retrieve(authEnv, "openai");
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        expect(result.value.total).toMatchObject({ current: 100, base: 200, override: null });
      }
    }
  );
});
