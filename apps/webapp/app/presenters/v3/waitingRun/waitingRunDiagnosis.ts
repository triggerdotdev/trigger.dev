/**
 * Deterministic "why is this run waiting?" diagnosis.
 *
 * There is no per-run start eta: queue order is offset by priority, sharded by concurrency key,
 * fair-randomly selected across queues, and the enqueue fast path skips the zset. `perRunStartEta`
 * ships an explicit `{ supported: false }` so absence can't be read as "unknown".
 *
 * The cause tree mirrors the health report's flow analyzer (reports/health/flow.ts): same
 * thresholds, same discriminator priority, same rule that a concurrency cause needs real
 * running-capacity evidence.
 */
...
/**
 * Queue signals over a recent window. Series and percentiles come from `queue_metrics_v1`; the
 * live "now" counts come from the run-queue's Redis counters, never a Postgres aggregate.
 */
...
export type WaitingRunDiagnosis = {
  run: {
    ...
    isWaiting: boolean;
    /**
     * False when `queuedAt` isn't this run's queue wait: resumed and retried runs keep a stale
     * `queuedAt`, so the number is reported but flagged.
     */
...
/**
 * A drain ETA ships only when the rate behind it is real: enough dequeues that it isn't one lucky
 * event, and enough buckets that the window isn't a single blip.
 */
...
/**
 * Statuses whose `queuedAt` is a leftover from the first enqueue: resume and retry re-enqueues
...
function describeRun(run: WaitingRunRow, now: Date): WaitingRunDiagnosis["run"] {
  ...
  if (run.queuedAt && queueWaitReliable) {
    // Queue wait is startedAt - queuedAt, or now - queuedAt for a run that hasn't started.
    ...
    // A delayed run isn't in the queue yet, so this is a schedule, not queue latency.
    ...
    // The delay elapsed but the run isn't enqueued. Creation age would hide the passed schedule.
    const ms = Math.max(0, now.getTime() - run.delayUntil.getTime());
    waitingLabel = `delay elapsed ${formatMs(ms)} ago, not yet enqueued`;
    waitingBasis = "delay_until";
  } else {
    // No queuedAt to measure from, so report creation age and say so in the label.
...
function summarizeQueue(
  ...
  // Prefer the live counter, then the freshest measured bucket rather than a confident zero.
  const lastMeasured =
...
function deriveCause(
  ...
  // Without both a real limit and a real running reading, the share is a meaningless 0 that would
  // falsely select `stall`. Same guard as flow.ts.
  ...
  // A queue's own limit shows up as throttling: `throttled_count` counts emissions where
  // running >= limit while work was queued. Sustained reads as pinned, intermittent as throttled.
  ...
  // Priority order from flow.ts: stall is a last resort, so a known config bottleneck (queue
  // throttling) must rule it out first.
  if (
  ...
  // Backlogged with nothing conclusive. With concurrency evidence in hand this is inconclusive,
  // not blind, so don't report evidence as missing.
  return {
    cause: "unknown",
...
// The drain ETA is a statement about the queue, never about this run.
function deriveDrainEta(
  ...
  // An unnamed cause means the evidence is incomplete, so don't dress it up as an ETA.
