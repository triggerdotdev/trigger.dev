...
// during staged rollout â€” see the comment above the claim block in
...
export class IdempotencyKeyConcern {
  ...
  // for any failure or miss â€” buffer outages must not 500 the trigger
  ...
  async handleTriggerRequest(
    ...
    // Buffer fallback per the mollifier-idempotency design. PG missed â€”
    ...
    // Pre-gate claim â€” closes the PG+buffer race during gate transition.
    // All same-key triggers serialise here before evaluateGate decides
    // PG-pass-through vs mollify. Skipped for triggerAndWait
    // (resumeParentOnCompletion) â€” that path bypasses the gate entirely
    ...
    // Redis SETNX for EVERY idempotency-keyed trigger â€” including orgs
    ...
    // flag is a pure in-memory read of `Organization.featureFlags` â€” no
    // DB query, same predicate the gate uses â€” keeping the claim's Redis
    ...
    if (claimEligible) {
      ...
      if (outcome.kind === "resolved") {
        ...
        // Claim resolved to a runId nothing can find â€” the run was
        ...
        // on â€” the PG unique constraint on the idempotency key
        // (RunDuplicateIdempotencyKeyError â†’ retry resolves to the
