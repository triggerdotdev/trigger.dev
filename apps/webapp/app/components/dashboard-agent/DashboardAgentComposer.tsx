...
export function DashboardAgentComposer({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "border border-border-bright/50 bg-background-bright transition focus-within:border-ring",
      ...
      >
        <div className={isHero ? "flex flex-col gap-1.5" : "flex items-end gap-1"}>
          <textarea
            ...
            className={cn(
              "max-h-[40vh] flex-1 resize-none border-0 bg-transparent px-1.5 py-0.5 text-sm leading-[1.5] text-text-bright placeholder-text-dimmed outline-hidden ring-0 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-surface-control field-sizing-content focus:outline-hidden focus:ring-0",
              isHero && "w-full"
            )}
          />
          {isHero ? (
            <div className="flex min-w-0 items-center justify-between gap-2">
              {context ?? <span />}
              {/* `shrink-0`, so a long context truncates instead of squeezing Send out. */}
