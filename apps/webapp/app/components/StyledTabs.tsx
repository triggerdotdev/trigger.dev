...
export function UnderlinedList({ children, ...props }: HeadlessTabListProps) {
  return (
    <HeadlessTab.List
      className={"-mb-px flex space-x-4 border-b border-slate-300"}
      {...props}
    >
      {children}
    </HeadlessTab.List>
      ...
      className={({ selected }: { selected: boolean }) =>
        classnames(
          selected
            ...
            : "border-transparent text-slate-300 hover:border-slate-200 hover:text-slate-200",
          "disabled:text-slate-300 disabled:hover:border-transparent",
          "flex whitespace-nowrap border-b-2 py-2 px-4 text-xs font-medium transition"
        )
      }
      {...props}
    >
