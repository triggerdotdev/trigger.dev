...
export function useDraggableResizable({
  ...
  const [rect, setRect] = useState<Rect>(() => {
    ...
    return clampRectToViewport({ ...initial, ...size }, getViewport(), viewportPadding, minSize);
  });
  ...
  const { w: minWidth, h: minHeight } = minSize;
  useEffect(() => {
    const onResize = () => {
      setRect((current) =>
        clampRectToViewport(current, getViewport(), viewportPadding, { w: minWidth, h: minHeight })
      );
    };
    onResize();
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, [viewportPadding, minWidth, minHeight]);
