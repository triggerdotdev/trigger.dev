...
const small = {
  container:
    "flex items-center h-6 gap-x-1.5 rounded hover:bg-tertiary pr-1 py-[0.1rem] pl-1.5 hover:disabled:bg-charcoal-700 transition focus-custom disabled:opacity-50 text-text-dimmed hover:text-text-bright disabled:hover:cursor-not-allowed hover:cursor-pointer disabled:hover:text-rose-500",
  root: "h-3 w-6",
...
const variations = {
  ...
  "tertiary/small": {
    ...
    root: cn(
      small.root,
      "group-data-[state=unchecked]:bg-charcoal-600 group-hover:group-data-[state=unchecked]:bg-charcoal-500/50"
    ),
    thumb: small.thumb,
    text: cn(
      small.text,
  ...
  "secondary/small": {
    ...
    root: cn(
      small.root,
      "group-data-[state=unchecked]:bg-charcoal-550 group-hover:group-data-[state=unchecked]:bg-charcoal-500"
    ),
    thumb: small.thumb,
...
export const Switch = React.forwardRef<React.ElementRef<typeof SwitchPrimitives.Root>, SwitchProps>(
  ({ className, variant, label, labelPosition = "left", ...props }, ref) => {
    ...
    const switchElement = (
      <div
        className={cn(
          "inline-flex shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors group-disabled:cursor-not-allowed group-disabled:opacity-50 group-data-[state=checked]:bg-blue-500 group-data-[state=unchecked]:bg-charcoal-700 group-hover:group-data-[state=unchecked]:bg-charcoal-500/50",
          root
        )}
      >
