...
export class SessionsReplicationService {
  ...
  async #flushBatch(flushId: string, batch: Array<SessionInsert>) {
    ...
    await startSpan(this._tracer, "flushBatch", async (span) => {
      const preparedInserts = batch.map((item) => ({
        organizationId: item.session.organizationId,
        row: toSessionInsertArray(item.session, item._version, item.event === "delete"),
      }));

      const routeCache = new Map<string, ClickHouse>();
      const groups = new Map<ClickHouse, SessionInsertArray[]>();

      for (const { organizationId, row } of preparedInserts) {
      ...
      for (const [clickhouse, group] of groups) {
        sortSessionInserts(group);
        combinedSessionInserts.push(...group);

        const [err] = await this.#insertWithRetry(
          (attempt) => this.#insertSessionInserts(clickhouse, group, attempt),
          "session inserts",
          flushId
        );
        if (err && !sessionError) {
        ...
        if (!err) {
          this._sessionsInsertedCounter.add(group.length);
        }
      }
