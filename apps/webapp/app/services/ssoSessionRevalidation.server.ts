...
export async function revalidateSsoSession(
  ...
  try {
    acquired = await redis.set(key, "1", "EX", interval, "NX");
  } catch (error) {
  ...
  // Browser navigations (and Remix client-side data requests, which
  // handle redirects via x-remix-redirect) get the logout redirect.
  // Programmatic/API fetches get a clean 401 instead of a 302-to-HTML.
