...
export async function findRunByIdWithMollifierFallback(
  ...
  try {
    ...
    return {
      ...
      // `batchId`. The nested `id` is the batch's internal cuid â€” the same
      // value PG stores in `TaskRun.batchId` â€” so callers reconstruct the
      ...
      // (`parentTaskRunId` / `rootTaskRunId` â€” what engine.trigger consumes),
      // not the friendlyIds the SyntheticRun contract expects. Convert
      // internal â†’ friendly here so consumers don't have to special-case
      // the buffered path.
    ...
    logger.error("mollifier read-fallback errored â€” fail-open to null", {
      runId: input.runId,
