...
export type ClaimOrAwaitOutcome =
  ...
  // so the buffer can compare-and-act against our ownership marker â€” a
...
// the trigger entirely â€” the caller responds with the cached runId.
...
export async function claimOrAwait(input: ClaimOrAwaitInput): Promise<ClaimOrAwaitOutcome> {
  ...
  if (!buffer) {
    // Mollifier disabled / buffer construction failed. Fall open â€”
    ...
    // for idempotency-keyed triggers â€” `triggerTask` is the
  ...
  // â€” we're the same logical claimant across attempts; only the slot
  ...
  // result.kind === "pending" â€” wait/poll loop. May see the value flip
  // to "resolved" (winner published), the key vanish (winner released
  // on error â†’ retry claim), or stay "pending" until the safety net.
  ...
  while (now() < deadline) {
    ...
    try {
      ...
      // Transient read failure â€” keep polling until deadline.
    ...
    if (current === null) {
      // Claimant released on error. Re-attempt the claim â€” one of the
    ...
    // current.kind === "pending" â†’ keep polling.
  }
...
// TTL is the safety net â€” waiters time out, SDK retries.
