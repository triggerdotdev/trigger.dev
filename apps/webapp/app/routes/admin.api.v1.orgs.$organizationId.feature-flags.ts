...
import { stripOrgForbiddenFlags, validatePartialFeatureFlags } from "~/v3/featureFlags";
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    ...
    // Drop only the global-only forbidden flags; merge semantics preserve every other override.
    const requestedFlags = stripOrgForbiddenFlags(rawRequestedFlags as Record<string, unknown>);
