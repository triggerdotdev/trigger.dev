...
export const AGENT_ORB_PALETTE = ["#3b82f6", "#f43f5e", "#22c55e", "#f59e0b"];
...
// Rejection-sample `count` points inside the trigger.dev logo silhouette, so a
// dense cloud reads as the solid logo at small sizes. Cached per count (uses a
// canvas hit-test, so client-only; returns [] during SSR).
const logoCloudCache = new Map<number, Pt[]>();
function logoCloud(count: number): Pt[] {
  ...
  while (pts.length < count && attempts < maxAttempts) {
    attempts++;
    const x = Math.random() * LOGO_W;
    const y = Math.random() * LOGO_H;
    if (ctx.isPointInPath(path, x, y, "evenodd")) {
      pts.push([(x - LOGO_W / 2) * scale, (y - LOGO_H / 2) * scale]);
    }
  }
  logoCloudCache.set(count, pts);
  return pts;
}
...
export function AgentOrb({
  ...
  const restPoints = useMemo(
    () => (restShape === "logo" ? logoCloud(effDotCount) : triangleOutline(effDotCount)),
    [restShape, effDotCount]
  );
...
export type AgentLogoMorphProps = {
  size?: number;
  active?: boolean;
  activation?: number;
  colors?: string[];
...
export function AgentLogoMorph({
  size = 20,
  active = false,
  activation,
  colors = AGENT_ORB_PALETTE,
