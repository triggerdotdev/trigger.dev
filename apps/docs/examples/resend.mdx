...
To use the Resend integration you must install the required packages:
<Tabs>
  <Tab title="npm">

Trigger.dev SDK & Resend packages for a new project:

```bash
npm install @trigger.dev/sdk @trigger.dev/resend zod
```
Resend packages for an existing project:
...
```bash
pnpm install @trigger.dev/sdk @trigger.dev/resend zod
```
Resend packages for an existing project:
```bash
...
```bash
yarn add @trigger.dev/sdk @trigger.dev/resend zod
```
Resend packages for an existing project:
