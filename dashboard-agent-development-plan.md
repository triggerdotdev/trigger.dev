...
- **M1 demo mode was superseded:** demo chats were removed in favor of a seeded live playground project (`db:seed:agent-examples` — real runs/errors/queues/deploys, live trigger:// links, example conversations, `--degrade/--recover` stand controls). Fixture matrices live only in the storybook gallery. The `--heartbeat` review-stand crutch was removed before merge; `seed-watch-scenarios.mts` replaced the hand-run scenario surgery. 
