...
- **M1 demo mode was superseded:** demo chats were removed in favor of exercising the agent against a real local project. Fixture matrices live only in the storybook gallery. The review/demo seeder was removed before merge; the scenario kit (`pnpm --filter webapp run scenarios:watch`, any project via `--project/--env`) replaced the hand-run scenario surgery. 
