...
| [Improved middleware & locals](/tasks/overview#improved-middleware-and-locals) | Our task middleware system is now much more useful. Previously it only ran "around" the `run` function, but now we've hoisted it to the top level and it now runs before/after all the other hooks. |
