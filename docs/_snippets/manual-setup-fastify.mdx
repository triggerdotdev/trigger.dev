...
To begin, install the necessary packages in your Fastify Server project directory. You can choose one of the following package managers:

<CodeGroup>

```bash npm
npm i @trigger.dev/sdk @trigger.dev/fastify
```
...
Register the Trigger Dev Fastify plugin, follow these steps based on your project's file type and structure
...
// register trigger dev fastify plugin with the client.
...
2. Inside the `jobs` folder, add a file named `example.(ts/js)`.
...
Build and Run your fastify server locally, like you normally would. For example:
