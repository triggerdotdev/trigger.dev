...
1. Create a new file named `+server.(ts/js)` within the `routes/api/trigger` directory.
...
Start your Sveltekit project locally, and then execute the `dev` CLI command to run Trigger.dev locally. You should run this command every time you want to use Trigger.dev locally.

![Your first Job](/images/cli-dev.gif)

<Warning>
  Make sure your Sveltekit site is running locally before continuing. You must also leave this `dev`
