...
Start your Astro project locally, and then execute the `dev` CLI command to run Trigger.dev locally. You should run this command every time you want to use Trigger.dev locally.

![Your first Job](/images/cli-dev.gif)

<Warning>
  Make sure your Astro site is running locally before continuing. You must also leave this `dev`
...
<Note>
  ...
  `--hostname <host>`. Example, in case your Astro.js is running on 0.0.0.0: `--hostname 0.0.0.0`.
</Note>
