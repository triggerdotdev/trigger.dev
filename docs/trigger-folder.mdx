...
Mostly this means you shouldn't need to think about what gets bundled. Just write your tasks and we'll take care of the rest. If you need to alter the bundling you use the [trigger.config file](/trigger-config).
...
It is possible to manually set one or more folders as `/trigger` folders in your [trigger.config file](/trigger-config).
