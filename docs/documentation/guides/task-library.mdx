...
// get it again later
const value = await io.store.job.get<string>("💾", "disk-A");
```

If you want to acess the store from outside a run (e.g. just from your backend) you should use [client.store](/sdk/triggerclient/store) instead.
