...
<Warning>
  Create a named **Trigger only** API key in your Development environment and set it as
  `TRIGGER_SECRET_KEY` in your `.env` or `.env.local` file. See the [API keys page](/apikeys)
