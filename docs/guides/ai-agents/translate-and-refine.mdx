...
This example is based on the "Evaluator-optimizer" pattern from Anthropic's [blog post](https://www.anthropic.com/research/building-effective-agents), where one LLM generates a response while another provides evaluation and feedback in a loop. This is particularly effective for tasks with clear evaluation criteria where iterative refinement provides measurable value.
...
This example payload will translate the text into French and should be suitably difficult to require a few iterations.