...
Then, check your [cloud.trigger.dev](http://cloud.trigger.dev) project 'Runs' table <Icon icon="circle-1" iconType="solid" size={20} color="A8FF53" /> and you should see a succesful `hello-world` task <Icon icon="circle-2" iconType="solid" size={20} color="A8FF53" /> which has been triggered when you added a new row with the `name` `Sarah Connor` to your `skynet` Supabase table.
