...
This guide will show you how to use [Trigger.dev](http://Trigger.dev) with Supabase edge functions.

- A simple Hello World example; a task is triggered when the edge function URL is accessed
...
<Note>
If you are only using Deno, to install Trigger.dev you must add a tsconfig file with these settings to the root:
```json tsconfig.json
{
  "compilerOptions": {
    "allowJs": true,
...
We will call this example `hello-world`.
...
Replace the `hello-world` plceholder code with the following:
...
<Note>
  ...
  you are using a different runtime like Deno. And do not add `npm:` to imports inside your task
