---
title: "Using Supabase edge functions with Trigger.dev"
sidebarTitle: "Supabase"
description: "This guide will show you how to setup Trigger.dev in your existing Supabase project, trigger an example task from a Supabase edge function, and then view the run."
icon: "N"
---
...
- _A simple Hello World example_; a task is triggered when the edge function URL is accessed

### Overview

Supabase edge functions allow you to trigger tasks when an event is sent from a third party (e.g. Stripe payments, when a new user signs up etc), or when there are any changes / updates to your Supabase database.
...
We will call this example `edge-function-simple`.

`supabase functions new edge-function-simple`

</Step>

<Step title="Update the edge function code">

Replace the `edge-function-simple` code with the following code:
