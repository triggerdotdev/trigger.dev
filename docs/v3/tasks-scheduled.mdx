...
They are defined in your code using `schedules.task()`. Then you need to attach a schedule to the task either in the schedules section of the dashboard or by using the SDK. You can provide an `externalId` when registering a schedule, this allows you to have schedules for your users. A task can have multiple schedules attached to it.
...
export const firstScheduledTask = schedules.task({
  ...
  run: async (payload) => {
    ...
    //elapsed time since last run (can be undefined if it's the first run)
    const msSinceLastRun = payload.lastTimestamp
      ? payload.timestamp.getTime() - payload.lastTimestamp.getTime()
      : undefined;
    console.log(msSinceLastRun);
