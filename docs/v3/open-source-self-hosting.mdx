...
Because self-hosted deployments can be highly tailored, we don't provide specific recommendations for scaling up or improving security and reliability.

If you've had enough of self-hosting, you can always join us on [Trigger.dev cloud](https://trigger.dev/pricing) where we deal with all these concerns so you don't have to.
...
```bash
git clone https://github.com/triggerdotdev/docker && cd docker
```
...
2. Tunnelling is not required for the worker components, but you will still have to set the `TRIGGER_PROTOCOL` and `TRIGGER_DOMAIN` as above.
