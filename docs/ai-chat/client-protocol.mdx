...
# 2. Subscribe to .out and read until trigger:turn-complete.
#    The doc's self-contained parser (Step 2) shows full SSE handling;
#    this recipe just greps for `text-delta` / `trigger:turn-complete` and
#    extracts the last seq_num so step 4 can resume from it.
SSE=$(curl -sS --max-time 30 -N \
  -H "Authorization: Bearer $PAT" \
  -H "Accept: text/event-stream" \
  -H "Timeout-Seconds: 20" \
  "$BASE_URL/realtime/v1/sessions/$SESSION_ID/out")
echo "$SSE" | grep -E 'text-delta|trigger:turn-complete' | head -2
...
curl -sS --max-time 30 -N \
  ...
  "$BASE_URL/realtime/v1/sessions/$SESSION_ID/out" \
  | grep -m 2 -E 'text-delta|trigger:turn-complete'
```
...
1. **Take refreshed tokens from the stream.** Every `trigger:turn-complete` chunk on `.out` includes a fresh `publicAccessToken` (see [`trigger:turn-complete`](#trigger-turn-complete)). For active conversations this just rolls — replace your stored token whenever the chunk carries one.
...
<Note>
  The built-in SDK clients (`TriggerChatTransport` from `@trigger.dev/sdk`, `AgentChat` from `@trigger.dev/sdk/chat`) call this endpoint and persist the body's `publicAccessToken` automatically, refreshing on every `trigger:turn-complete`.
</Note>
...
| `records[].body` | A JSON-encoded **string** (not an object) wrapping `{ data: UIMessageChunk, id: string }`. You must `JSON.parse(body)` to get the chunk. |
...
If you're working in TypeScript and depending on `@trigger.dev/core/v3` is acceptable, use `SSEStreamSubscription` — it handles batch decoding, deduplication, and `Last-Event-ID` tracking for you:
    ...
    for (const raw of events) {
      ...
      if (eventType === "batch") {
        const batch = JSON.parse(data) as {
          records: Array<{ seq_num: number; timestamp: number; body: string }>;
        };
...
for await (const { seqNum, chunk } of readSessionOut(sseUrl, publicAccessToken)) {
  lastSeq = String(seqNum);
  if ((chunk as { type: string }).type === "trigger:turn-complete") break;
...
| `finish-step` / `finish` | end markers for the assistant message | Followed by `trigger:turn-complete`. |

Refer to the AI SDK docs linked above for the full union — only the two chunks below are Trigger.dev-specific.
...
Signals that the agent cannot handle this message on its current version and a new run must be started. Emitted when the agent calls [`chat.requestUpgrade()`](/ai-chat/patterns/version-upgrades) before processing the turn.
...
| `publicAccessToken` | JWT for session access. Stable across runs; refreshed via `trigger:turn-complete` chunks. |
...
Yes. `.in` records are processed in arrival order — the agent's stop handler aborts the in-flight `streamText`, emits `trigger:turn-complete`, and reads the next record. You don't have to wait for `trigger:turn-complete` on the wire before posting the next `.in/append`. In practice you usually do anyway, because your UI is gated on the stream coming back to ready.
</Expandable>
...
- A `trigger:turn-complete` means **the turn finished**, not that the run is gone. The run may still be alive, idle-waiting for the next `.in` record, or it may have suspended / exited shortly after.
...
If you genuinely need the live `runId` (for displaying the dashboard link, say), read it from the latest `trigger:turn-complete`'s refreshed `publicAccessToken` — the JWT's `read:runs:{runId}` scope encodes it. Or call `GET /api/v1/sessions/{sessionId}` (omitted from this page; see the Sessions API reference) to read `currentRunId`.
</Expandable>
