...
On deployments that enable it (Trigger.dev Cloud, or a self-hosted instance backed by real S2), the SDK can read the response stream straight from the realtime stream store instead of relaying it through Trigger.dev, removing a hop on the latency-sensitive path. Reading `.out` directly does not affect this pattern's trust model: `.out` carries no injected signal, so it stays read-only and can't bypass the `.in`/session-create injection the worker performs.

<Note>
Because this pattern sets a custom `baseURL`, the direct read is **off by default** here — `.out` follows your routing (to Trigger in the split, or through your worker in the all-through variant), exactly as before and with no extra config, so nothing changes when you update the SDK. To opt the split's `.out` into the faster direct-from-stream-store path, set `directStreamReads: true` on the transport; reads then go straight to the stream store (read-only, so the trust model is unaffected) and fall back to the relayed route on reconnect or token expiry. The `fetch` override still wraps the direct request if you'd rather funnel it through your own edge.
</Note>
