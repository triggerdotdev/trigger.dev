...
| `triggerConfig` | `SessionTriggerConfig` | Trigger options applied to every run: `tags` (up to 10, same as [run tags](/tags); for `chat.agent` the auto-added `chat:{chatId}` tag uses one slot), `queue`, `machine`, `maxAttempts`, `idleTimeoutInSeconds`, `basePayload`. |
