...
When the deployment enables direct reads (Cloud, or self-hosted backed by real S2), the SDK reads the active turn's `.out` directly from the stream store, removing a hop on the streaming path. It's automatic and falls back to the proxied read on reconnect or once the read token expires. Setting a custom `baseURL` (above) automatically turns the direct read off so your `.out` routing is honored — no extra config needed. Pass `directStreamReads: true` to force it back on, or `false` to force the proxied route.
