...
| `headStart`            | `string`                                                             | —                           | URL of a [`chat.headStart`](/ai-chat/head-start) route handler. When set, the FIRST message of a brand-new chat POSTs to this URL so step 1's LLM call runs in your warm process while the agent run boots in parallel. Subsequent turns bypass it. |
