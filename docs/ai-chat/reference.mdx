...
| `directStreamReads`    | `boolean`                                                            | auto                        | Read the active turn's `.out` directly from the stream store when the server returns a read grant (Cloud, or self-hosted backed by real S2). Defaults on, but **off automatically when you set a custom `baseURL`/`streamBaseURL`** so your `.out` routing (e.g. your own edge proxy) is honored without any extra config. Set `true` to force it on even with a custom `baseURL`, or `false` to force the proxied route. |
...
| `directStreamReads`    | `boolean`                                                            | auto                                   | Read `.out` directly from the stream store when the server returns a read grant (Cloud, or self-hosted backed by real S2). Defaults on, but off automatically when you set a custom `baseURL` so your routing is honored. Set `true`/`false` to force it. |
