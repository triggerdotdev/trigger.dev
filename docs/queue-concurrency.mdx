...
To bound keyed runs as a group, set `combinedConcurrencyLimit`. Each key still gets at most `concurrencyLimit`, and keyed runs as a whole never exceed the combined limit; runs triggered without a key sit outside it:
