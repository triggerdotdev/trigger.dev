...
By default the hook replays the channel history, then live-tails. Pass `from: "latest"` to receive only records appended after you connect, and `maxRecords` to bound memory:

```tsx
const { records } = useSessionStream<{ url: string }>(sessionId, {
  accessToken,
  io: "out",
  from: "latest", // only records appended after this component connects
