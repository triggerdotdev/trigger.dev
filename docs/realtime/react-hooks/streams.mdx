...
By default a subscriber replays the whole stream history, then live-tails. Pass `from: "latest"` to skip history and receive only records appended after you connect, and `maxParts` to keep memory bounded. Together they give a last-value view:
...
export function LatestFrame({ runId, accessToken }: { runId: string; accessToken: string }) {
  const { parts } = useRealtimeStream<{ url: string }>(runId, "frames", {
    accessToken,
    from: "latest", // only frames appended after this component connects
...
```tsx
const saved = localStorage.getItem("frames-cursor") ?? undefined;
...
const { parts, lastEventId } = useRealtimeStream<{ url: string }>(runId, "frames", {
  ...
  onParts: (batch) => localStorage.setItem("frames-cursor", batch.at(-1)!.id),
});
```
