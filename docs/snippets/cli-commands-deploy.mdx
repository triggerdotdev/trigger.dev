...
<ParamField body="Native build" type="--native-build">
  Build the image on the native build server, ignoring the build path configured for the project on the server.
</ParamField>

<ParamField body="Depot build" type="--depot-build">
  Build the image with Depot, ignoring the build path configured for the project on the server.
</ParamField>
