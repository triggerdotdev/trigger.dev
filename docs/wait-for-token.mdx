...
<Note>
  If you're waiting for data from an [input stream](/tasks/input-streams), use [`inputStream.wait()`](/tasks/input-streams#wait--suspend-until-data-arrives) instead — it uses waitpoint tokens internally but provides a simpler API with full type safety from your stream definition.
</Note>
