...
If the task definition sets a [`region` option](/tasks/overview#region-option), the region you pass here must be one of the task's regions, otherwise the trigger is rejected. Without a per-trigger region, the project default is used when the task allows it, otherwise the first region the task lists.
