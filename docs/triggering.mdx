...
Trigger your tasks **from your backend**:
...
| `tasks.triggerAndPoll()`         | Anywhere              | Triggers a single run of a task with the payload you pass in, and any options you specify, and then polls the run until it’s complete. [Read more](#tasks-triggerandpoll) |

Trigger your tasks **from inside a run**:
