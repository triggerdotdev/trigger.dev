...
export abstract class ScalingStrategy {
  ...
  abstract calculateTargetCount(context: ScalingContext): number;
...
export class NoneScalingStrategy extends ScalingStrategy {
  ...
  constructor(_options?: ScalingStrategyOptions) {
    super();
    // No-op - none strategy doesn't use options
  }

  calculateTargetCount(context: ScalingContext): number {
    return context.maxConsumerCount;
  }
}
...
export class AggressiveScalingStrategy extends ScalingStrategy {
  ...
  calculateTargetCount(context: ScalingContext): number {
    ...
    const queuePerConsumer = smoothedQueueLength / currentCount;

    // Define zones based on targetRatio
    // Optimal zone: 0.5x to 2x the target ratio
    const scaleDownThreshold = context.targetRatio * 0.5;
    const scaleUpThreshold = context.targetRatio * 2.0;
    ...
    if (queuePerConsumer < scaleDownThreshold) {
      ...
      return Math.max(Math.floor(currentCount * reductionFactor), context.minConsumerCount);
      ...
      if (queuePerConsumer >= context.targetRatio * 5) {
        // Critical: Queue is 5x target ratio or higher
        scaleFactor = 1.5; // 50% increase
      } else if (queuePerConsumer >= context.targetRatio * 3) {
      ...
      return Math.min(currentCount + maxIncrement, targetCount, context.maxConsumerCount);
    } else {
