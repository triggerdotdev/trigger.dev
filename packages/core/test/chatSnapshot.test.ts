...
describe("parseTranscriptSnapshot", () => {
  ...
  it("drops version 2 entries whose message is null or not an object", () => {
    const parsed = parseTranscriptSnapshot({
      version: 2,
      savedAt: 10,
      messages: [
        { id: "u-1", final: true, message: user },
        { id: "bad-1", final: true, message: null },
        { id: "bad-2", final: false, message: "oops" },
        { id: "", final: true, message: assistant },
        { id: "a-1", final: true, message: assistant },
      ],
      state: null,
    });

    expect(parsed?.messages.map((m) => m.id)).toEqual(["u-1", "a-1"]);
  });

  it("drops version 2 entries whose message.id disagrees with the envelope id", () => {
    const parsed = parseTranscriptSnapshot({
      version: 2,
      savedAt: 10,
      messages: [
        { id: "u-1", final: true, message: user },
        { id: "mismatch", final: true, message: assistant },
        { id: "a-1", final: true, message: assistant },
      ],
      state: null,
    });
