...
describe("the persistence gate split", () => {
  ...
  it("hands a head-start first turn to loadContext as incoming messages, without seeding them twice", async () => {
    const calls: { incoming: string[]; previous: string[] }[] = [];
    const stored: UIMessage[] = [];
    const loadContext = async (
      _scope: unknown,
      event: { incomingMessages: UIMessage[]; previousMessages: UIMessage[] }
    ) => {
      calls.push({
        incoming: event.incomingMessages.map((m) => m.id),
        previous: event.previousMessages.map((m) => m.id),
      });
      for (const m of event.incomingMessages) {
        if (!stored.some((s) => s.id === m.id)) stored.push(m);
      }
      return [...stored];
    };
    ...
    const agent = chat.agent({
      id: "gate-split-head-start-load-context",
    ...
    const harness = mockChatAgent(agent, {
      chatId: "gate-split-head-start-load-context",
    ...
    try {
      ...
      await waitFor(() => roles !== undefined, "turn complete");

      expect(calls).toHaveLength(1);
      expect(calls[0]!.incoming).toEqual(["hs-user-1"]);
