...
describe("SessionOutputChannel initializeSessionStream cache", () => {
  it("dedupes repeated pipe()/writer() calls for the same channel", async () => {
    ...
    expect(spy).toHaveBeenCalledWith("session-1", "out", undefined, undefined, undefined);
  });
  ...
  it("scopes the cache per channel instance", async () => {
    ...
    expect(spy).toHaveBeenCalledTimes(2);
    expect(spy).toHaveBeenCalledWith("session-a", "out", undefined, undefined, undefined);
    expect(spy).toHaveBeenCalledWith("session-b", "out", undefined, undefined, undefined);
  });
